...
* 
* **Exclusión Mutua Avanzada (Mutex Reentrante):** Se implementó la primitiva de sincronización `threading.RLock()` (Reentrant Lock) en el servidor central. Esta elección de diseño previene de forma absoluta los interbloqueos (*Deadlocks*) por auto-bloqueo cuando un hilo invoca métodos protegidos de forma anidada (por ejemplo, al ejecutar `shutil.copy()` y llamar inmediatamente a `registrar_log()`, compartiendo el mismo contexto de exclusión mutua). 

* **Estrategia de Bloqueo Híbrida:** * **En el Servidor (`servidor.py`):** Se adoptó un enfoque seguro de grano grueso para la manipulación de archivos y logs dentro del mismo hilo, asegurando que las operaciones de copia (`shutil.copy`) y el registro de auditoría sean completamente atómicos y consistentes.
  * **En el Demonio (`demonio.py`):** Se utiliza un enfoque de grano fino, donde la exclusión mutua se restringe estrictamente al milisegundo en que se modifica el set de control en la memoria RAM (`archivos_en_proceso`) y el archivo central de auditoría, dejando las operaciones pesadas de disco (`shutil.move()`) corriendo en paralelo fuera del entorno crítico.
